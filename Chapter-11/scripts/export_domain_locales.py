...
DOMAIN_FIELD_MAP = {
    ...
    "customer_service": {
        "zh": {
            "central_agent_system": ("domains.customer_service.prompts", "CENTRAL_AGENT_SYSTEM_PROMPT"),
            "aggregation": ("domains.customer_service.prompts", "AGGREGATION_PROMPT"),
            "facts_prompt": ("domains.customer_service.prompts", "FACTS_PROMPT"),
            "decomposition_prompt": ("domains.customer_service.prompts", "DECOMPOSITION_PROMPT"),
            "dependency_system": ("domains.customer_service.prompts", "DEPENDENCY_SYSTEM_PROMPT"),
            "dependency_user": ("domains.customer_service.prompts", "DEPENDENCY_USER_PROMPT"),
            "agent_routing": ("domains.customer_service.prompts", "AGENT_ROUTING_PROMPT"),
            "supervisor_system": ("domains.customer_service.prompts", "SUPERVISOR_SYSTEM_PROMPT"),
            "multi_task_title": ("__const__", "📋 客服处理结果"),
            "single_task_title": ("__const__", "📋 客服回复"),
            "aggregation_skip_hint": ("__const__", "单意图咨询，直接使用子智能体回复"),
            "memory_aggregation_instruction": (
                "__const__",
                "综合子任务结果回复用户，仅包含与原始问题相关的内容。",
            ),
        },
        "en": {
            "central_agent_system": ("domains.customer_service.prompts_en", "CENTRAL_AGENT_SYSTEM_PROMPT"),
            "aggregation": ("domains.customer_service.prompts_en", "AGGREGATION_PROMPT"),
            "facts_prompt": ("domains.customer_service.prompts_en", "FACTS_PROMPT"),
            "decomposition_prompt": ("domains.customer_service.prompts_en", "DECOMPOSITION_PROMPT"),
            "dependency_system": ("domains.customer_service.prompts_en", "DEPENDENCY_SYSTEM_PROMPT"),
            "dependency_user": ("domains.customer_service.prompts_en", "DEPENDENCY_USER_PROMPT"),
            "agent_routing": ("domains.customer_service.prompts_en", "AGENT_ROUTING_PROMPT"),
            "supervisor_system": ("domains.customer_service.prompts_en", "SUPERVISOR_SYSTEM_PROMPT"),
            "multi_task_title": ("__const__", "Customer Service Summary"),
            "single_task_title": ("__const__", "Customer Service Reply"),
            "aggregation_skip_hint": ("__const__", "Single-intent query; use sub-agent reply directly"),
            "memory_aggregation_instruction": (
                "__const__",
                "Synthesize subtask results for the user; include only content relevant to the original question.",
            ),
        },
    },
}
...
def main() -> None:
    for domain in ("travel", "customer_service"):
