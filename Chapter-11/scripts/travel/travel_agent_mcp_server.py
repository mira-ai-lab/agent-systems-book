"""stdio / SSE MCP Server：travel 能力展示域（书稿 Fixed Graph 全链路）。

产品推荐入口请用 ``python scripts/run_demo.py``（默认 travel + profile=auto）。
...
Claude Code 注册（stdio，推荐）：
    cd Chapter-8
    claude mcp add travel-agent --scope project -- python scripts/travel/travel_agent_mcp_server.py
...
HTTP/SSE 模式（可选，供远程 MCP 客户端）：
    ...
    python scripts/travel/travel_agent_mcp_server.py
...
_CHAPTER8_ROOT = Path(__file__).resolve().parent.parent.parent
...
mcp = FastMCP("travel-example", host=_HOST, port=_PORT)
