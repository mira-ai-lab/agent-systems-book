"""从 Chapter-6 父目录加载模块，避免 Chapter-6/langgraph 目录名遮蔽 pip langgraph 包"""
...
# 这些模块会间接 import langchain → langgraph，加载时不能让 Chapter-6 在 sys.path 中
...
def load_ch6_module(name: str) -> ModuleType:
    ...
    if name in _LANGCHAIN_DEPENDENT:
        # 移除 Chapter-6，防止 `import langgraph` 命中 Chapter-6/langgraph/ 目录
        sys.path[:] = [
            p for p in sys.path
...
def import_pip_langgraph(submodule: str = "") -> ModuleType:
    """导入 site-packages 中的 langgraph"""
    import importlib

    blocked = {str(CHAPTER6_DIR), str(CHAPTER6_DIR / "langgraph")}
    saved = sys.path[:]
