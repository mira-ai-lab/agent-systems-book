...
使用 **LangGraph Supervisor**（`create_supervisor` + handoff）实现与 `langgraph/` 等价的旅行多智能体能力。

## 与 langgraph/ 对比

| 维度 | `langgraph/` | `supervisor/` |
|------|--------------|---------------|
...
`langgraph/LangGraphOrchestrator` 同样支持 `long_term_backend` 参数。
...
| **规划流水线** | TaskPlanner 拆解为 **>1** 个子任务 | `planned_pipeline.py`：`build_plan` + `execute_layer`（同 `langgraph/`） |
