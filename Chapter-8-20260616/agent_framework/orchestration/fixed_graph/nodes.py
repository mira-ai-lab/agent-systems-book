...
def _message_chunk_text(chunk: Any) -> str:
    """从 AIMessageChunk / 消息对象提取可流式输出的文本。"""
    if chunk is None:
        return ""
...
def _chunk_has_tool_call(chunk: Any) -> bool:
    """是否为 tool-call 流式 chunk（非用户可见的自然语言回答）。"""
    if chunk is None:
        return False
    return bool(getattr(chunk, "tool_calls", None) or getattr(chunk, "tool_call_chunks", None))


def _extract_agent_text_from_message(message: Any) -> str:
    """提取 Agent 自然语言回答；跳过纯 tool-call 轮次。"""
    if message is None or _chunk_has_tool_call(message):
        return ""
    if getattr(message, "tool_calls", None):
        return ""
    return _message_chunk_text(message).strip()


async def _read_agent_text_from_state(agent: Any, config: Dict[str, Any]) -> str:
    """从 Agent checkpoint 取最后一条非 tool-call 的 AI 文本。"""
    ...
    try:
        ...
        for message in reversed(messages):
            if getattr(message, "type", None) != "ai":
                continue
            text = _extract_agent_text_from_message(message)
            if text:
                return text
    except Exception:
        return ""
...
async def _collect_sub_agent_via_stream(
    agent: Any,
    ...
    *,
    ...
    """通过 astream_events 执行子 Agent，并逐 token 推送 graph.subtask.token。"""
    tool_outputs: List[Any] = []
    agent_text = ""
    ...
    async for event in agent.astream_events(input_state, config, version="v2"):
        ...
        if kind == "on_chat_model_stream":
            ...
            if _chunk_has_tool_call(chunk):
                continue
            token = _message_chunk_text(chunk)
            if token:
                stream_sink.emit_subtask_token(task_id, agent_name, token)
        elif kind == "on_chat_model_end":
            text = _extract_agent_text_from_message(event.get("data", {}).get("output"))
            if text:
                agent_text = text
        elif kind == "on_tool_end":
            data = event.get("data") or {}
            tool_name = event.get("name") or data.get("name") or f"{agent_name}.tool"
            raw_output = data.get("output")
            content = getattr(raw_output, "content", raw_output)
            parsed_content = _parse_tool_output(content)
    ...
    if not agent_text:
        agent_text = await _read_agent_text_from_state(agent, config)

    return tool_outputs, agent_text
...
async def _invoke_sub_agent(
    ...
    try:
        ...
        agent_config = {"configurable": {"thread_id": f"{thread_id}_{task_id}"}}
        input_state = {"messages": [("user", query_text)]}
        use_token_stream = (
            stream_sink is not None
            and stream_sink.enabled
            and hasattr(agent, "astream_events")
        )

        tool_outputs: List[Any] = []
        agent_text = ""
        if use_token_stream:
            tool_outputs, agent_text = await _collect_sub_agent_via_stream(
                agent,
                input_state,
                agent_config,
                task_id=task_id,
            ...
            agent_state = await agent.ainvoke(input_state, agent_config)
            for msg in agent_state.get("messages", []):
                ...
                if msg.type == "tool" and hasattr(msg, "content"):
                    ...
                    text = _extract_agent_text_from_message(msg)
                    if text:
                        agent_text = text

        if not agent_text:
            agent_text = await _read_agent_text_from_state(agent, agent_config)
...
class GraphContext:
    ...
    registry / prompts / domain_config 必须由编排器注入，框架无领域默认回落。
    """
    ...
    def __init__(
        ...
        if registry is None or prompts is None or domain_config is None:
            raise ValueError(
                "GraphContext 需要 registry、prompts、domain_config；"
                "请通过 LangGraphOrchestrator 或 compile_graph 注入领域组件。"
            )
...
def make_nodes(ctx: GraphContext):
    ...
    async def pre_survey_node(state: CentralAgentState) -> Dict[str, Any]:
        ...
        prefilled = state.get("prefilled_pre_survey")
        mode = ctx.pipeline.resolved_pre_survey_mode
        if prefilled and mode == PRE_SURVEY_MODE_ROUTER_PREFILL:
            pre_survey = dict(prefilled)
            summary = {k: v for k, v in pre_survey.items() if k != "raw_text"}
            logs = _append_log(
                {**state, "logs": logs},
                "✓ 使用 RouterEngine 预填 pre_survey（跳过 LLM 预调查）",
            )
            if not _streaming(state):
                logs = _append_log(
                    {**state, "logs": logs},
        ...
        if prefilled and mode == PRE_SURVEY_MODE_FULL_CH2:
            logs = _append_log(
                {**state, "logs": logs},
                "✓ 完整 Ch2 LLM 预调查（覆盖 Router 轻量 pre_survey）",
            )
        summary = {k: v for k, v in pre_survey.items() if k != "raw_text"}
        if _streaming(state):
    ...
    async def build_plan_node(state: CentralAgentState) -> Dict[str, Any]:
        ...
        logs = _append_log(state, "\n📋 [Ch4] 任务拆解 → 依赖分析 → 子 Agent 路由...")
        prefilled = state.get("prefilled_execution_plan")
        if prefilled:
            plan = dict(prefilled)
            logs = _append_log(
                {**state, "logs": logs},
                "✓ 使用 RouterEngine 预填 execution_plan（跳过 TaskPlanner）",
            )
            if ctx.pipeline.resolved_pre_survey_mode == PRE_SURVEY_MODE_FULL_CH2:
                ch2 = state.get("pre_survey") or {}
                plan["pre_survey"] = {
                    k: v for k, v in ch2.items() if k != "raw_text"
                }
                plan["pre_survey_raw"] = ch2.get("raw_text", "")
                plan["pre_survey_source"] = "task_planner_llm"
        else:
            if not ctx.pipeline.allow_task_planner_decomposition:
                raise ValueError(
                    "Router 统一路径需要 prefilled_execution_plan；"
                    "请通过 RouterOrchestrator 或显式传入 prefilled_execution_plan"
                )
    ...
    async def execute_layer_node(state: CentralAgentState) -> Dict[str, Any]:
        ...
        subtask_stream_sink = ctx.stream_sink if _streaming(state) else None
        ...
        if len(tasks) == 1:
            layer_results = [
                await _invoke_sub_agent(
                    ...
                    stream_sink=subtask_stream_sink,
                )
            ]
        else:
        ...
        for res in layer_results:
            ...
            if _streaming(state):
                ctx.stream_sink.emit_subtask_completed(res)
